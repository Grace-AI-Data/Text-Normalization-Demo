  ...
  *   `addressing.FreenessAllocator` for keeping track of memory usage, where
  ...
  def _read_weights(self, inputs, memory, prev_read_weights, link):
    ...
    with tf.name_scope(
      ...
      return (tf.expand_dims(content_mode, 2) * content_weights + tf.reduce_sum(
          tf.expand_dims(forward_mode, 3) * forward_weights, 2) + tf.reduce_sum(
              tf.expand_dims(backward_mode, 3) * backward_weights, 2))
